...
class Entry:
    ...
    def localize(self,fluor_image):
        ...
        dim0_list=list(range(0,fluor_image.shape[0],a))
        dim1_list=list(range(0,fluor_image.shape[1],b))
        if dim0_list[-1]!=fluor_image.shape[0]:
            dim0_list.append(fluor_image.shape[0])
        if dim1_list[-1]!=fluor_image.shape[1]:
        ...
        #assert len(offsets0)==len(offsets1), f"{len(offsets0)=}!={len(offsets1)=}"
        ...
        for i in dim0_list:
            for j in dim1_list:
                ...
                #fluor_image[from_ax0[0]:from_ax0[1],from_ax1[0]:from_ax1[1]]=0
        ...
        #f"total number of snippets required for prediction of a whole frame: {total_num_snippets=} {batch_size=}"
        if total_num_snippets<batch_size:
            ...
            em_out.xyz_px[:,0]+=offsets1[em_out.frame_ix]
            em_out.xyz_px[:,1]+=offsets0[em_out.frame_ix]
