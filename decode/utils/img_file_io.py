...
def read_img(path,from_dtype='u12',to_dtype="float32"):
    ...
    assert from_dtype in ("u8","u12",'u16',"float","float32")
    assert to_dtype in ("u8","u12",'u16',"float","float32")

    if from_dtype==to_dtype:
        return img_data

    if to_dtype in ("float","float32"):
        if from_dtype=="u12":
...
def write_img(path,img_data:numpy.ndarray,from_dtype="float32",as_dtype:str="u16"):
    assert isinstance(img_data,numpy.ndarray)
    assert img_data.dtype in (numpy.uint8,numpy.uint16,numpy.float32)
    assert as_dtype in ("u8","u12","u16")
    
    if from_dtype in ("float32","float"):
